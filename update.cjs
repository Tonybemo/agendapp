...
// 1. Create Inicio.jsx
const inicioContent = `import React, { useState, useEffect } from 'react';
...
fs.writeFileSync('src/pages/Inicio.jsx', inicioContent, 'utf8');
console.log('Created src/pages/Inicio.jsx');

// 2. Modify src/pages/Dashboard.jsx
let dashboardContent = fs.readFileSync('src/pages/Dashboard.jsx', 'utf8');

// Change path for Tratamientos
...
// Change importe formula
dashboardContent = dashboardContent.replace(
  /importe: \(totalHoras \* 11\)\.toFixed\(0\),/,
  `importe: (totalExtras * 11).toFixed(0),`
);

const searchWrapperIndex = dashboardContent.indexOf(`<div className="dashboard-search-wrapper"`);
if (searchWrapperIndex !== -1) {
  const finalReturn = dashboardContent.substring(0, searchWrapperIndex).trim() + '\n    </div>\n  );\n};\n\nexport default Dashboard;\n';
  dashboardContent = finalReturn;
}
fs.writeFileSync('src/pages/Dashboard.jsx', dashboardContent, 'utf8');
console.log('Updated src/pages/Dashboard.jsx');

// 3. Modify src/pages/Aquapp.jsx
let aquappContent = fs.readFileSync('src/pages/Aquapp.jsx', 'utf8');

if (!aquappContent.includes('useLocation')) {
  aquappContent = aquappContent.replace(
    /import { useNavigate } from 'react-router-dom';/,
    `import { useNavigate, useLocation } from 'react-router-dom';`
  );
}

if (!aquappContent.includes('new URLSearchParams(location.search)')) {
  const activeTabLine = "const [activeTab, setActiveTab] = useState('historial');";
  const activeTabReplacement = `const location = useLocation();
  ...
  aquappContent = aquappContent.replace(activeTabLine, activeTabReplacement);
}
fs.writeFileSync('src/pages/Aquapp.jsx', aquappContent, 'utf8');
console.log('Updated src/pages/Aquapp.jsx');

// 4. Update src/App.jsx
let appContent = fs.readFileSync('src/App.jsx', 'utf8');
if (!appContent.includes('import Inicio')) {
  appContent = appContent.replace(
    /import Dashboard from '\.\/pages\/Dashboard';/,
    `import Dashboard from './pages/Dashboard';\nimport Inicio from './pages/Inicio';`
  );
}
if (!appContent.includes('<Route path="/inicio"')) {
  appContent = appContent.replace(
...
// 5. Update src/components/Layout.jsx
let layoutContent = fs.readFileSync('src/components/Layout.jsx', 'utf8');
const bottomNavRegex = /const bottomNavItems = \[[\s\S]*?\];/;
const newBottomNav = `const bottomNavItems = [
...
layoutContent = layoutContent.replace(bottomNavRegex, newBottomNav);

layoutContent = layoutContent.replace(
  /{ path: '\/', label: 'Inicio', icon: <LayoutDashboard size={20} \/>, public: true },/,
  `{ path: '/', label: 'Dashboard', icon: <BarChart2 size={20} />, public: true },
    { path: '/inicio', label: 'Inicio', icon: <LayoutDashboard size={20} />, public: true },`
);
fs.writeFileSync('src/components/Layout.jsx', layoutContent, 'utf8');
console.log('Updated src/components/Layout.jsx');
